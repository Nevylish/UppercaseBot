...
export class Dashboard {
    ...
    private setupRoutes(): void {
        ...
        this.app.post('/api/guilds/:guildId/invite', async (req, res) => {
            /**
             * For context: By default, UpperCase Bot don't have the permissions to see or create invitations.
             *              If someone add full permissions to him I can generate invitation but members can see me if I join.
             *              The Dashboard access is restricted. No one malicious or curious can join your guild.
             *              UpperCase Bot don't have access to members list¹ or messages content².
             *
             *              1 : UpperCase Bot can't see the members names, avatar or everything. Only the guild's owner informations.
             *              2 : UpperCase Bot can't see your conversations. Even if he have Administrator permissions.
            ...
            try {
                const results = await this.manager.broadcastEval(
                    async (client, context) => {
                        ...
                        if (!context.createNew) {
                            ...
                            return { needsNewInvite: true };
                        }

                        const me = guild.members.me;
                        if (!me?.permissions.has('CreateInstantInvite')) {
                            return { error: "Le bot n'a pas la permission de créer des invitations" };
                        }

                        try {
                            const channel = guild.channels.cache.filter((c) => c.type === context.channelType).first();

                            if (!channel || !('createInvite' in channel)) {
                                return { error: "Aucun salon textuel trouvé pour créer l'invitation" };
                            }
                    ...
                    { context: { guildId, createNew, channelType: ChannelType.GuildText } },
                );
                ...
                if (result.needsNewInvite) {
                    return res.status(202).json({ needsNewInvite: true });
                }

                res.json({ url: result.url });
            } catch (err) {
                Logger.error('Dashboard', 'Error when creating invite link\n', err);
